...
function dbNotify () {
  return got.post('http://localhost:3030/dbChange')
    .catch(e => console.error('Error: tigris-server not responding'))
}
...
function run () {
  ...
  const serverLog = `tigris-admin-server listening on port ${serverPort}`
  const supportLog = `tigris-support-server listening on port ${supportPort}`
