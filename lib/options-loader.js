...
function load () {
  ...
  try {
    const globalConfigPath = path.resolve('/etc', 'tigris', 'tigris.json')
    return {
    ...
    try {
      const homeConfigPath = path.resolve(os.homedir(), '.tigris', 'tigris.json')
      return {
        path: homeConfigPath,
        opts: JSON.parse(fs.readFileSync(homeConfigPath))
      }
    } catch (_) {
      console.error("Couldn't open tigris.json config file.")
      process.exit(1)
    }
  }
}
